#!/usr/bin/env bash
# Run zoom-only SFT on Qwen2.5-VL-7B using the parquets produced by
#   python -m sft.build_dataset.build --target zoom_only
# By default those parquets live under <project_root>/sft/sft_dataset/zoom_only/.
set -xeuo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
PROJECT_ROOT=$(cd "${SCRIPT_DIR}/../.." && pwd)
DATA_ROOT=${DATA_ROOT:-${PROJECT_ROOT}/sft/sft_dataset/zoom_only}
...
EXP_NAME=${EXP_NAME:-qwen2_5_vl_7b_sft_zoom_only}
