    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # this is experimenfftal and may be changed/removed in the future in favor of a general-purpose one
                if isinstance(self.train_dataloader.sampler, AbstractCurriculumSampler):
                    self.train_dataloader.sampler.update(batch=batch)

                # ==========================================
                # Read metrics from batch.non_tensor_batch
                # ==========================================

                reward_data = batch.non_tensor_batch

                weight_keys = ["w_acc", "w_base_fmt", "w_tool_fmt", "w_penalty"]
                for key in weight_keys:
                    if key in reward_data:
                        # Weights are usually 1D, so we take [0]
                        val = float(reward_data[key][0]) if isinstance(reward_data[key], (list, np.ndarray)) else float(reward_data[key])
                        metrics[f"weights/{key}"] = val

                reward_components = ["r_acc", "r_base_fmt", "r_tool_fmt", "r_penalty"]
                for key in reward_components:
                    if key in reward_data:
                        vals = np.asarray(reward_data[key], dtype=np.float32)
                        if len(vals) > 0:
                            metrics[f"training-aux/{key}_mean"] = float(np.mean(vals))

                if "num_tool_calls" in reward_data and "valid_tool_calls" in reward_data:
                    num_calls_arr = np.asarray(reward_data["num_tool_calls"], dtype=np.float32)
                    valid_calls_arr = np.asarray(reward_data["valid_tool_calls"], dtype=np.float32)

                    total_num_calls = np.sum(num_calls_arr)
                    total_valid_calls = np.sum(valid_calls_arr)

                    if len(num_calls_arr) > 0:
                        metrics["training-aux/num_tool_calls_mean"] = float(np.mean(num_calls_arr))
                        
                    if total_num_calls > 0:
                        metrics["training-aux/tool_format_success_rate"] = float(total_valid_calls / total_num_calls)
                    else:
                        metrics["training-aux/tool_format_success_rate"] = 0.0

                # ==========================================
                # Log model response samples for inspection
                # ==========================================
                if "model_response" in reward_data:
                    responses = reward_data["model_response"]
                    prompts = reward_data.get("question", reward_data.get("prompt", [""] * len(responses)))
                    acc_arr = reward_data.get("r_acc", [])
                    tools_arr = reward_data.get("num_tool_calls", [])

                    num_samples_to_log = min(3, len(responses))

                    # 1. 打印到控制台 (保持原始终端可见性)
                    formatted_samples_str = f"\\n\\n{'='*20} STEP {self.global_steps} SAMPLES {'='*20}\\n"
                    table_rows = [] # 准备给后端 Logger 的结构化数据

                    for i in range(num_samples_to_log):
                        resp_text = str(responses[i])
                        prompt_text = str(prompts[i]) if i < len(prompts) else "N/A"
                        acc_val = float(acc_arr[i]) if i < len(acc_arr) else 0.0
                        tools_val = float(tools_arr[i]) if i < len(tools_arr) else 0.0

                        # 拼接终端字符串
                        formatted_samples_str += (
                            f"\\n--- Sample {i+1} ---\\n"
                            f"[Prompt Preview] : {prompt_text[:200]}...\\n"
                            f"[Metrics]        : Accuracy = {acc_val} | Tool Calls = {tools_val}\\n"
                            f"[Model Response] :\\n{resp_text}\\n"
                            f"{'-'*50}\\n"
                        )
                        # 收集结构化数据行
                        table_rows.append([self.global_steps, i+1, prompt_text, resp_text, acc_val, tools_val])

                    print(formatted_samples_str)

                    table = wandb.Table(
                        columns=["Step", "Sample", "Prompt", "Response", "Accuracy", "Tool Calls"],
                        data=table_rows
                    )
                    wandb.log({"Training_Responses_Table": table}, step=self.global_steps, commit=False)
